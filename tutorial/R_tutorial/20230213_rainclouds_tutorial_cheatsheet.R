...
#answer*
ggplot(simdat, aes(x=group,y=score, fill = group)) +
  geom_rain(rain.side = "l", violin.args = list(adjust = 2))
...
#*answer
#*
ap5.1_r <- ggplot(simdat_round,aes(x=group,y=score,fill=group)) +
  geom_rain(point.args.pos = list(), point.args = list(alpha = .4)) +
  # geom_rain(point.args.pos = list(alpha = .4)) # this would work but it is not proper convention
  scale_fill_brewer(palette = "Dark2") +
  guides(fill = "none", col = "none")

ap5.2_r <- ggplot(simdat_round,aes(x=group,y=score,fill=group)) +
  geom_rain(point.args = list(alpha = .4)) +
  scale_fill_brewer(palette = "Dark2") +
  guides(fill = "none", col = "none")
p5_r <- ap5.1_r + ap5.2_r + plot_annotation("Figure 8: Jittering Ordinal Data", tag_levels = "a")
p5_r
...
#*answer
ggplot(rep_data, aes(x = time, y = score, fill = group, color = group)) +
  geom_rain(alpha = .8,
            boxplot.args = list(color = "black", outlier.color = NA),
            violin.args = list(adjust = 1.5, trim = FALSE),
            point.args.pos = list(position = ggpp::position_jitternudge(width = 0.04, height = 0, x = -.2,
                                                                        y = 0, nudge.from = "jittered")),
            boxplot.args.pos = list(position = ggpp::position_dodgenudge(x = .0), width = .2)) +
  scale_color_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  ggtitle("Figure 10: Repeated Measures Factorial Rainclouds with geom_rain")
...
ggplot(rep_data, aes(x = time, y = score, fill = group, color = group)) +
  geom_rain(boxplot.args = list(color = "black", outlier.color = NA),
            violin.args = list(adjust = 1.5, trim = FALSE),
            point.args.pos = list(position = ggpp::position_jitternudge(width = 0.04, height = 0, x = -.15,
                                                                        y = 0, nudge.from = "jittered")),
            boxplot.args.pos = list(position = ggpp::position_dodgenudge(x = .0), width = .1)) +
  geom_line(data = sumrepdat, aes(x = as.numeric(time)+.1, y = score_mean, group = group, colour = group), linetype = 3)+
  geom_point(data = sumrepdat, aes(x = as.numeric(time)+.1, y = score_mean, group = group, colour = group), shape = 18) +
  geom_errorbar(data = sumrepdat, aes(x = as.numeric(time)+.1, y = score_mean, group = group, colour = group, ymin = score_mean-score_sem_ci, ymax = score_mean+score_sem_ci), width = .05)+
  scale_colour_brewer(palette = "Dark2")+
...
ggplot(rep_data, aes(x = time, y = score, fill = group, color = group)) +
  geom_rain(id.long.var = "id",
            line.args = list(color = "black", alpha = .2),
            boxplot.args = list(color = "black", outlier.color = NA),
            violin.args = list(adjust = 1.5, trim = FALSE),
            boxplot.args.pos = list(position = ggpp::position_dodgenudge(x = .1), width = .1)) +
